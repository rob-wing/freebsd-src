...
#ifdef _KERNEL
MALLOC_DEFINE(M_EBPF, "eBPF", "eBPF instructions");
#endif
...
static bool convert_bpf_load(const struct bpf_insn *fp,
			     struct ebpf_insn **new_insnp __unused)
{
	switch (fp->k) {
			      ...
			      struct ebpf_insn *new_prog, uint32_t *new_len)
{
	...
	if (new_prog) {
#ifdef _KERNEL
		addrs = malloc(len * sizeof(*addrs), M_EBPF, M_WAITOK | M_ZERO);
#else
...
#define BPF_EMIT_JMP							\
		...
		case BPF_MISC | BPF_TXA:
			*insn = BPF_MOV64_REG(BPF_REG_A, BPF_REG_X);
			...
			/* A = wirelen or X = wirelen */
		case BPF_LD | BPF_W | BPF_LEN:
		case BPF_LDX | BPF_W | BPF_LEN:
#define BPF_LDX_WIRELEN(CLASS, DST, SRC)			\
	((struct ebpf_insn) {					\
		.code  = CLASS | BPF_W | BPF_LEN,		\
		.dst_reg = DST,					\
		.src_reg = SRC,					\
		.off   = 0,					\
		.imm   = 0 })
			*insn = BPF_LDX_WIRELEN(BPF_CLASS(fp->code), BPF_CLASS(fp->code) == BPF_LD ?
...
struct rte_bpf_prm *
ebpf_convert_classic(const struct bpf_program *prog)
{
...
#ifdef _KERNEL
	prm = malloc(sizeof(*prm) + ebpf_len * sizeof(*ebpf), M_EBPF, M_WAITOK | M_ZERO);
#else
	prm = malloc(sizeof(*prm) + ebpf_len * sizeof(*ebpf));
#endif

	if (prm == NULL)
		return (NULL);

	/* The eBPF instructions in this case are right after the header */
	ebpf = (void *)(prm + 1);
	...
	prm->prog_arg.type = RTE_BPF_ARG_PTR_MBUF; /* needed for verifying */
	prm->prog_arg.size = sizeof(struct mbuf);
